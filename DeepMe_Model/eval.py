...
def save_results(predictions, targets, time_indices, time_ids,
                 mse, rmse, mae, num_valid_samples, r2, r, model_used):
    ...
    with open('eval_results/summary_report.txt', 'w') as f:
        ...
        f.write(f"Evaluation Date: {evaluation_date}\n\n")
        ...
        if num_valid_samples > 0:
            ...
            for _, row in worst_predictions.iterrows():
                f.write(
                    f"  TimeID {row['TimeID']}: "
                    f"Actual={row['Actual']:.4f}, "
                    f"Predicted={row['Predicted']:.4f}, "
                    f"Error={row['Error']:.4f}\n"
                )


def save_full_predictions_for_fold(predictions, targets, batch_indices,
                                   time_indices, time_ids, fold_id):

    os.makedirs('eval_results', exist_ok=True)

    fold_time_indices = time_indices[batch_indices]

    full_results_df = pd.DataFrame({
        'TimeIndex': fold_time_indices,
        'TimeID': time_ids[fold_time_indices],
        'Actual': targets,
        'Predicted': predictions,
        'IsValidTarget': (targets != 0).astype(int)
    })

    out_path = os.path.join('eval_results', f'full_predictions_fold{fold_id}.csv')
    full_results_df.to_csv(out_path, index=False)
    print(f"  Saved full predictions for Fold {fold_id} to: {out_path}")


def main():
    print("\n" + "=" * 50)
    print("Starting Model Evaluation (5-fold models)")
    print("=" * 50)
    ...
    fold_paths = [f"best_model_fold{i}.pth" for i in range(1, 6)]

    all_metrics = []
    best_rmse = float('inf')
    best_info = None

    for fold_id, model_path in enumerate(fold_paths, start=1):
        print("\n" + "-" * 50)
        print(f"Evaluating model: Fold {fold_id} | Path: {model_path}")
        ...
        print(f"  Fold {fold_id} Metrics (Non-Zero Targets Only):")
        print(f"    MSE:  {mse:.6f}")
        ...
        save_full_predictions_for_fold(
    ...
    if best_info is None:
        print("\nNo valid models were evaluated. Please check model paths.")
        return


    print("\n" + "=" * 50)
    print("Summary over all evaluated folds:")
    for m in all_metrics:
        print(f"  Fold {m['fold']} | RMSE={m['rmse']:.6f}, MAE={m['mae']:.6f}, "
              f"R2={m['r2']:.6f}, R={m['r']:.6f}")

    print("\nBest model selected based on RMSE:")
    print(f"  Fold {best_info['fold']} | Path: {best_info['model_path']}")
    print(f"  RMSE={best_info['rmse']:.6f}, MAE={best_info['mae']:.6f}, "
          f"R2={best_info['r2']:.6f}, R={best_info['r']:.6f}")
    print("=" * 50)


    best_predictions = best_info['predictions']
    best_targets = best_info['targets']
    best_batch_indices = best_info['batch_indices']

    all_time_indices = time_indices[best_batch_indices]
    non_zero_mask = (best_targets != 0)

    visualize_results(best_predictions, best_targets, all_time_indices, time_ids, non_zero_mask)
    save_results(
        best_predictions,
        best_targets,
        all_time_indices,
        ...
        best_info['rmse'],
        best_info['mae'],
        best_info['num_valid_samples'],
        best_info['r2'],
        best_info['r'],
        best_info['model_path']
    )

    print("\nEvaluation completed!")
    print("Full predictions for all folds saved as: eval_results/full_predictions_fold*.csv")
    print("Plots + detailed_predictions.csv + summary_report.txt generated ONLY for the best model.")
    print(f"Best model: Fold {best_info['fold']} ({best_info['model_path']})")
    print(f"Final Metrics (Non-Zero Targets): "
          f"MSE={best_info['mse']:.4f}, RMSE={best_info['rmse']:.4f}, "
          f"MAE={best_info['mae']:.4f}, R2={best_info['r2']:.4f}, R={best_info['r']:.4f}")
