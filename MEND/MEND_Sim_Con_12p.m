...
clear all;
clc;
% read litterfall and soilT and soilM files
...
st=stc; % define control/heat trt
...
lf=lfc; % define control/heat trt 
% smc = xlsread('NARR_HF_soilM_19892010_hourly.csv','B:B'); % control Mpa
% smt = xlsread('NARR_HF_soilM_19892010_hourly.csv','D:D'); % heated Mpa
M = readmatrix('NARR_HF_soilM_19892010_hourly.csv');
smc = M(:,2);
smh = M(:,4);
sm=smc; % define control/heat trt  % 12/2017 added moisture
rho = 1; % cm-3 g soil-1
...
par=[0.31  -0.008	0.00028   0.50   0.50   11.7 43  1.0  0.55  0.35 0.0014 0.0014]';
Min=[0.00  -0.017	-0.002    0.30   0.30   1    30  0.1  0.02  0.02 0.0001 0.0001]';  % 12/2017 added wider priors
Max=[0.72   0.017	0.002     0.70   0.70   23   55  1.9  0.90  0.90 0.0070 0.0070]';  % 07/2018 added wider priors

% par=[0.31  -0.008            0.00028   0.50   0.50   11.7 43  1.0  0.55  0.35 0.0014 0.0014]';
% 
% Min=[0.00  -0.017           -0.002    0.30   0.30   1    30  0.1  0.02  0.02 0.0001 0.0001]';  % 12/2017 added wider priors
% 
% Max=[0.60   0.017           0.002     0.70   0.70   23   55  1.9  0.90  0.90 0.0070 0.0070]';  
...
% ends
% cost function 
% use SOC, MBC, DOC, ENC, CO2
 J_old= nansum((mod1SOC-obscSOC).^2./SOCd_obsSD) + nansum((mod1MBC-obscMBC).^2./MBCd_obsSD) + nansum((mod1DOC-obscDOC).^2./DOCd_obsSD) + nansum((mod1ENC-obscENC).^2./ENCd_obsSD) + nansum((mod1CO2d-obscCO2d).^2./CO2d_obsSD)+nansum((mod1CO2h-obscCO2h).^2./CO2h_obsSD);
% USE SOC, MBC, DOC, CO2, no ENC !
%J_old= nansum((mod1SOC-obscSOC).^2./SOCd_obsSD) + nansum((mod1MBC-obscMBC).^2./MBCd_obsSD) + nansum((mod1DOC-obscDOC).^2./DOCd_obsSD) + nansum((mod1CO2d-obscCO2d).^2./CO2d_obsSD)+nansum((mod1CO2h-obscCO2h).^2./CO2h_obsSD);
% cost function ends
...
% use SOC, MBC, DOC, ENC, CO2
 J_new = nansum((mod2SOC-obscSOC).^2./SOCd_obsSD) + nansum((mod2MBC-obscMBC).^2./MBCd_obsSD) + nansum((mod2DOC-obscDOC).^2./DOCd_obsSD) + nansum((mod2ENC-obscENC).^2./ENCd_obsSD) + nansum((mod2CO2d-obscCO2d).^2./CO2d_obsSD)+nansum((mod2CO2h-obscCO2h).^2./CO2h_obsSD);
% USE SOC, MBC, DOC, CO2, no ENC !
%J_new = nansum((mod2SOC-obscSOC).^2./SOCd_obsSD) + nansum((mod2MBC-obscMBC).^2./MBCd_obsSD) + nansum((mod2DOC-obscDOC).^2./DOCd_obsSD) + nansum((mod2CO2d-obscCO2d).^2./CO2d_obsSD)+nansum((mod2CO2h-obscCO2h).^2./CO2h_obsSD);
% cost function ends
...
b=mle(Parameters_keep(1,ceil(upgrade/2):upgrade-1),'distribution','gev');
c=mle(Parameters_keep(2,ceil(upgrade/2):upgrade-1),'distribution','gev');
d=mle(Parameters_keep(3,ceil(upgrade/2):upgrade-1),'distribution','gev');
e=mle(Parameters_keep(4,ceil(upgrade/2):upgrade-1),'distribution','gev');
f=mle(Parameters_keep(5,ceil(upgrade/2):upgrade-1),'distribution','gev');
g=mle(Parameters_keep(6,ceil(upgrade/2):upgrade-1),'distribution','gev');
h=mle(Parameters_keep(7,ceil(upgrade/2):upgrade-1),'distribution','gev');
i=mle(Parameters_keep(8,ceil(upgrade/2):upgrade-1),'distribution','gev');
j=mle(Parameters_keep(9,ceil(upgrade/2):upgrade-1),'distribution','gev');
k=mle(Parameters_keep(10,ceil(upgrade/2):upgrade-1),'distribution','gev');
l=mle(Parameters_keep(11,ceil(upgrade/2):upgrade-1),'distribution','gev');
% exxtraction end
...
csvwrite('da_parameters_summary_mle_cont.csv',mleU);  % 12/2017 added
% mle selection ends
% mean para value extracted
% all para 
...
% two sets of means integrated
ZZ=[tmpALL,tmp];
csvwrite('da_parameters_summary_mean_cont.csv',ZZ);
% mean para ends
...
% 
% %Export final pool sizes or flux to .txt files
% fid=fopen('prediction_SOCfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', SOCfinal);
% fclose(fid);
% fid=fopen('prediction_MBCfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', MBCfinal);
% fclose(fid);
% fid=fopen('prediction_DOCfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', DOCfinal);
% fclose(fid);
% fid=fopen('prediction_ENCfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', ENCfinal);
% fclose(fid);
% fid=fopen('prediction_CO2dfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', CO2dfinal);
% fclose(fid); 
% fid=fopen('prediction_CO2hfinal_mle.txt','w');
% fprintf(fid,'%6.8f \r\n', CO2hfinal);
% fclose(fid); 
% % exports ends
% 
